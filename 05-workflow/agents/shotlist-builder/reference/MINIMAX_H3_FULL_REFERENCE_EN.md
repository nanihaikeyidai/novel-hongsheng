...
> The basic formats for shots, camera movement, speakers, dialogue, and ordinary sound are shared with the Video Prompt Writing Guide (I2VA / FL2VA / L2VA). This guide focuses on the reference labels, analysis sections, and format differences specific to full-reference mode.
...
The basic format follows the Video Prompt Writing Guide (I2VA / FL2VA / L2VA):
...
For complete rules and examples covering camera vocabulary, group speech, voice-over, dialogue across cuts, and visible text, see the Video Prompt Writing Guide (I2VA / FL2VA / L2VA).

### 5.2 Full-Reference Mode Differences

| Dimension | Base keyframe modes | Full-reference mode |
| --- | --- | --- |
...
The basic speaker-ID and `<d>` formats follow the base keyframe guide. When a referenced subject physically speaks, retain both the visual reference label and the speaker ID:
...
The subject label in this construction must define the actual vocal source. Before serialization, resolve `<Subject N>` back to its `subject_definitions` line. A person, character, narrator, creature, or explicitly defined vocal source may carry `(Sx)`; an environment, street, room, stall, skyline, style treatment, camera reference, prop, paper crane, or other non-vocal object may not. Never attach a speaker ID to whichever subject number happens to be nearby. If the real speaker has no visible subject reference, name the stable voice source directly and append `(Sx)` without borrowing another `<Subject N>`.

Every `<d>` event must have the correct `(Sx)` in the immediately preceding vocal clause. A later speaker tag elsewhere in the shot does not repair an unbound dialogue event. Apply the same subject-to-speaker mapping in the Chinese mirror.
...
The definitions of these two sound categories follow the Video Prompt Writing Guide (I2VA / FL2VA / L2VA).
