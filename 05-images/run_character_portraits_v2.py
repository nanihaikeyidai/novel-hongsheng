...
def build_prompt(positive_text, seed=-1):
    ...
    return {
        ...
        "63": {  # PrimitiveStringMultiline - user prompt
            "class_type": "PrimitiveStringMultiline",
            "inputs": {
                "value": positive_text
            }
        },
        "51": {  # CLIPTextEncode - positive
            "class_type": "CLIPTextEncode",
            "inputs": {
                "clip": ["56", 0],
                "text": [positive_text, 0]  # 从 widget 取文本
            }
        },
        "76": {  # CLIPTextEncode - negative
            "class_type": "CLIPTextEncode",
            "inputs": {
                "clip": ["56", 0],
                "text": NEGATIVE
...
outputs = submit_and_wait(prompt)
...
if outputs:
    ...
    if saved:
        ...
        for i, char in enumerate(characters[1:], 2):
            ...
            outputs = submit_and_wait(prompt, timeout=120)
        ...
        print("\n" + "=" * 60)
        print("【生成汇总】")
        print(f"  ✅ 林晓: {os.path.join(OUTPUT_DIR, '林晓_人设_上半身正面.png')}")
        for r in results:
            icon = "✅" if r['success'] else "❌"
            print(f"  {icon} {r['name']}: {os.path.join(OUTPUT_DIR, r['name'] + '_人设_上半身正面.png')}")
        print("=" * 60)
    else:
        print("  ⚠ 验证图保存失败")
else:
