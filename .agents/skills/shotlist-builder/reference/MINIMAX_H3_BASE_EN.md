# Video Prompt Writing Guide (I2VA / FL2VA / L2VA)

## 1. Task Overview

- **I2VA**: Builds a complete audiovisual timeline from a first-frame reference and develops forward from that image.
- **FL2VA**: Builds a complete audiovisual timeline connecting exact first- and last-frame references.
- **L2VA**: Builds a complete audiovisual timeline that converges from a plausible preceding state to an exact last-frame reference.
...
At the beginning of `[Shot 1]`, state the overall style and initial composition. Common styles include `Cinematic`, `live-action`, `2D-animated`, `3D CG`, `claymation`, `watercolor`, and `vintage film`. Derive the style from the submitted reference image and any explicit user direction.
