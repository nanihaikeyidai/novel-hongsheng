...
## 4. After approval — translating the schema into the selected platform

For **Seedance 2.0**, every prompt for that scene includes the original `⚠️空间布局` block that mirrors the approved schema:
...
For **MiniMax H3**, do not insert this Chinese warning block or `@imageN` handles into the canonical H3 prompt. Translate the same approved geometry into natural English inside `integrated_multimodal_description` or `detailed_description`, following [MINIMAX_H3_ROUTING.md](MINIMAX_H3_ROUTING.md).

For both platforms, preserve all of these facts:
- Distances in meters
