// Hollywood-format screenplay — .docx generator (LEGACY)
//
// Default skill output is Markdown (`markdown-style.md`). Use this script only
// when the user explicitly asks for a `.docx` file.
//
// Usage:
//   1. Copy this file into your working folder, rename it (e.g., build_my_scene.js).
//   2. Inside the `screenplay` array, write your scenes via helpers (slug/action/character/dial, etc.).
//   3. Run: NODE_PATH=/usr/local/lib/node_modules_global/lib/node_modules node build_my_scene.js
//   4. You get screenplay.docx in the same folder.
//
// Hollywood format:
//   Page 8.5" x 11" (US Letter)
//   Margins: top 1", bottom 1", left 1.5", right 1"
//   Courier New 12pt (monospaced → 1 page ≈ 1 minute of screen time on mixed dialogue/action)
...
// Toggle scene auto-numbering. Set the starting number if the block doesn't begin at 1.
...
const screenplay = [
  ...
  action("Description of what is visible in frame. Action verbs only."),
  action("A second action, if needed."),
